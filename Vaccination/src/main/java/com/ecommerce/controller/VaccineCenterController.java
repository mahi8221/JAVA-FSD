...
@Controller
@RequestMapping("/vaccine-centers") public class
VaccineCenterController {

private final VaccineCenterService vaccineCenterService; private final
CitizenService citizenService;

public VaccineCenterController(VaccineCenterService vaccineCenterService,
CitizenService citizenService) {
this.vaccineCenterService = vaccineCenterService; this.citizenService =
citizenService;
}
...
model.addAttribute("vaccineCenters", vaccineCenters); return "vaccinecenter-details";
}

@GetMapping("/add")
public String showAddVaccineCenterForm(Model model) {
model.addAttribute("vaccineCenter", new VaccineCenter()); return "addvaccine-center";
}

@PostMapping("/add")
public String addVaccineCenter(@ModelAttribute("vaccineCenter")
VaccineCenter vaccineCenter) {
vaccineCenterService.addVaccineCenter(vaccineCenter); return
"redirect:/vaccine-centers";
}
...
model.addAttribute("citizens", citizens); return "vaccine-center-view";
}

@GetMapping("/{id}/edit")
public String showEditVaccineCenterForm(@PathVariable("id") Long id, Model model) {
VaccineCenter vaccineCenter = vaccineCenterService.getVaccineCenterById(id); if
(vaccineCenter != null) {
model.addAttribute("vaccineCenter", vaccineCenter); return "editvaccine-center";
} else {
...
@ModelAttribute("vaccineCenter") VaccineCenter updatedVaccineCenter) { VaccineCenter
vaccineCenter = vaccineCenterService.getVaccineCenterById(id); if (vaccineCenter != null) {
...
vaccineCenterService.deleteVaccineCenter(id); return "redirect:/vaccinecenters";
}
}